...
; BORDER (1)
 ...
 LD IX, OBJECT_POINTS_00 ; +2 ; +(3*4)-2
...
Q_KEY_N  LD BC,$DFFE     ; Load BC with the row port address
         ...
         JP NZ,P_KEY_N   ; not pressed
                ...
                INC     hl
                LD      (x_offset), hl
P_KEY_N  LD BC,$FDFE     ; Load BC with the row port address
         IN A,(C)        ; Read the port into the accumulator
         AND $01         ; a
...
ObjectPointsResults DEFS 256, 0
...
ROTATE_POINT_LOOP
  ...
  ld sp, ix              ; move sp to next point
...
; bc=rotated x de=totated y hl=rotated z
...
;; perspective
  ...
  ld b, high RX_LO       ; point to RX_LO table
  ld a, (bc)             ; store PX_LO value
  add a, 96
  ...
  ld b, high RY_LO       ; point to RX_LO table
  ld a, (bc)             ; store PX_LO value
  add a, 128
...
DRAW_LOOP
  ...
  ld b, high PX_LO       ; perspective low table
  ld d, high PY_LO       ; perspective low table
