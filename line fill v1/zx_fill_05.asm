...
CLEAR_SCREEN LD (CLEAR_SCREEN_END+1),SP ; Store the stack (self modding code)
 ...
 LD DE,%101010101010101                ; set DE to 0
...
FILL_EDGE MACRO  ()
        ...
        LD      A,      (DE)                    ; get screen
        AND     (HL)
        INC     H                               ; ScrBufOrRight - todo - get this from b?
        OR      (HL)
        LD      (DE),   A                       ; put screen
        ...
        LD      A,      (DE)                    ; get screen
        AND     (HL)
        INC     H                               ; ScrBufORLeft - todo - get this from c?
        OR      (HL)
        LD      (DE),   A                       ; put screen
MEND

FILL_BYTE MACRO  ()
; hl points to the screen - de holds pattern

        ex      de,     hl                      ; right edge on screen
;        LD      H,      HIGH ScrANDRight
        ld      l,      b                       ; left pixel - todo - get this from ixl?
 ld b,h ; backup pattern
        LD      H,      HIGH ScrANDRight
...
; calc screen pos + b in h and l
        LD      H,      HIGH ScrBufL            ; SCREEN V_LO TABLE
        LD      L,      E                       ; A=VPOS
 ...
 ld (FILL_JMP+1), a
 ld de, %1111111111111111 ; 0101010101010101 ; pattern
; ld de, iy - iy holds pattern?

; todo - change to b+c hold pattern left and pattern right high table / ix holds left + right
